...
class KgAdapterTripsEncoder(nn.Module):
    pass
...
class KgAdapterCrossAttention(nn.Module):
    ...
    def forward(
        ...
        bsz, q_len, kv_len, hidden_size = q_hidden_states.size(), q_hidden_states.size(1), k_hidden_states.size(
        ...
        key_states = self.k_proj(k_hidden_states)
        value_states = self.v_proj(k_hidden_states)
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        self,
        hidden_states: torch.Tensor, #传递进来的是hidden_states
        attention_mask: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
        past_key_value: Optional[Cache] = None,
...
class LlamaWithKgAdapterEncLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        self.kg_adpater_info_merge = nn.Linear(config.kg_adapter_info_merge, config.hidden_size,
                                               in_encoder=True)
    ...
    def forward(
        ...
        #! --------------- kg_adapter ----------------
        sg_rep = update_flag_sg(sg,self.exp_set)
...
class LlamaKgAdapterModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        if self.use_edge_emb:
            self.kg_adapter_embed_edges = nn.Sequential(
                nn.Linear(config.num_relations + 1 + 5 * 2, config.kg_adapter_hidden_size),
                nn.LayerNorm(config.kg_adapter_hidden_size),
                ACT2FN[config.hidden_act],
                nn.Linear(config.kg_adapter_hidden_size, config.kg_adapter_hidden_size), 
            )
        # 线性投影层 B
        self.kg_adapter_egdes_proj = nn.Linear(config.kg_adapter_hidden_size, config.kg_adapter_hidden_size)
        # 混合因子： 可学习的参数，用于控制文本表示与 KG 表示的融合比例。
        self.kg_adapter_t2n_mix_factor = nn.Parameter(torch.zeros(1))
        self.kg_adapter_t2e_mix_factor = nn.Parameter(torch.zeros(1))
    ...
    def forward(
        ...
        t2n_gate = torch.sigmoid(self.kg_adapter_t2n_mix_factor)
        ...
        if self.mix_emb:
            if edges_embeds is None:
                ...
                if self.linear_emb:
                    ...
                    t2e_gate = torch.sigmoid(self.kg_adapter_t2e_mix_factor)
        ...
        ### 初始化隐藏层，形状为[batch_size, seq_len, hidden_size]
        hidden_states = inputs_embeds
        ...
        # 将32个decoder_layer展开，每个decoder_layer都是一个LlamaDecoderLayer
        for decoder_layer in self.layers:
            if output_hidden_states:
                all_hidden_states += (hidden_states,)
            ### 不断更新hidden_states
            if self.gradient_checkpointing and self.training: #节约现存
            ...
            else: ### 主要片段：decoder_layer的forward方法
                layer_outputs = decoder_layer(
                    # 先传递 hidden_states: [batch_size, seq_len, hidden_size]
                    hidden_states,
        ...
        # 将最后的hidden_states传入norm层，进行标准化
