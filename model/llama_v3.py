...
def _make_causal_mask(
    ...
    该函数用于生成因果掩码（causal mask），用于双向自注意力机制中，确保模型在生成下一个词时只能关注当前词及之前的词，防止信息泄露。
    
    •	input_ids_shape: 输入张量的形状，通常是 (batch_size, sequence_length)。
	•	dtype: 数据类型，例如 torch.float32。
	•	device: 计算设备，例如 CPU 或 GPU。
	•	past_key_values_length: 已有的键和值的长度，主要用于增量解码时的缓存。
    """
    # 这里将输入的形状拆分为批次大小 bsz 和目标序列长度 tgt_len。
    bsz, tgt_len = input_ids_shape
    # 创建一个大小为 (tgt_len, tgt_len) 的张量 mask，初始化为最小的浮点数值（负无穷接近值）。这个掩码将用于屏蔽后续位置的注意力。
    mask = torch.full((tgt_len, tgt_len), torch.tensor(torch.finfo(dtype).min, device=device), device=device)
    # 创建一个从 0 到 tgt_len - 1 的张量 mask_cond，用于生成掩码的条件。
    mask_cond = torch.arange(mask.size(-1), device=device)
    # 这实际上会生成一个下三角矩阵，包括对角线位置，表示当前时间步及之前的时间步
    mask.masked_fill_(mask_cond < (mask_cond + 1).view(mask.size(-1), 1), 0)
    mask = mask.to(dtype)
    # 如果有过去的键和值（例如在增量解码中），需要调整掩码的大小，在左侧拼接对应长度的零张量。
    if past_key_values_length > 0:
        mask = torch.cat([torch.zeros(tgt_len, past_key_values_length, dtype=dtype, device=device), mask], dim=-1)
    # 将掩码在批次维度上扩展到 bsz，使其适用于整个批次。
...
def _expand_mask(mask: torch.Tensor, dtype: torch.dtype, tgt_len: Optional[int] = None):
    ...
    该函数将输入的注意力掩码从形状 [batch_size, seq_len] 扩展为 [batch_size, 1, tgt_seq_len, src_seq_len]，用于兼容多头注意力机制的计算。
    
    •	mask: 输入的注意力掩码，形状为 [batch_size, seq_len]，通常由 1 和 0 组成，表示哪些位置需要被关注。
	•	dtype: 数据类型，例如 torch.float32。
	•	tgt_len: 目标序列的长度，如果为 None，则默认与源序列长度相同。
    """
    # 	获取批次大小 bsz 和源序列长度 src_len。如果 tgt_len 未指定，则令其等于 src_len。
    bsz, src_len = mask.size()
    tgt_len = tgt_len if tgt_len is not None else src_len
    # 将第三维度扩展到 tgt_len，形状变为 (batch_size, 1, tgt_len, src_len)
    expanded_mask = mask[:, None, None, :].expand(bsz, 1, tgt_len, src_len).to(dtype)
    # 掩码取反，即 1 变为 0，0 变为 1。
    inverted_mask = 1.0 - expanded_mask
    # • 将掩码中值为 1 的位置（需要屏蔽）填充为负无穷接近值，值为 0 的位置保持不变。
	# •	返回的 inverted_mask 用于在注意力计算中加到 scores 上，使被屏蔽的位置的注意力权重趋近于 0。
    return inverted_mask.masked_fill(inverted_mask.to(torch.bool), torch.finfo(dtype).min)


class RMSNorm(nn.Module):
    """Root Mean Square Layer Normalization.
    RMSNorm 类实现了根均方层归一化，与传统的 Layer Normalization 不同，RMSNorm 只使用向量的 RMS 值进行归一化，而不减去均值。这种方法可以降低计算成本，同时在某些情况下提高模型性能。
    """

    def __init__(self, size: int, dim: int = -1, eps: float = 1e-5) -> None:
        """
        •	size: 归一化的维度大小，即要归一化的特征数。
        •	dim: 进行归一化的维度，默认是最后一个维度（-1）。
        •	eps: 一个很小的数，防止除零错误，默认值为 1e-5。
        """
        super().__init__()
        # 可学习的缩放参数，初始化为全 1 的张量，大小为 size。
...
class LlamaRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        ...
        •	hidden_size: 隐藏层的大小，即特征维度的大小。
	    •	eps: 一个小的 epsilon 值，默认为 1e-6，用于数值稳定性。
        """
        super().__init__()
        # 可学习的缩放参数，大小为 hidden_size，初始化为全 1
    ...
    def forward(self, hidden_states):
        """
        hidden_states: 输入的隐藏状态张量，形状为 (batch_size, sequence_length, hidden_size)
        """
        # 计算方差:对最后一个维度（hidden_size 维度）上的元素进行平方，然后计算均值，得到每个位置的方差
        variance = hidden_states.to(torch.float32).pow(2).mean(-1, keepdim=True)
        hidden_states = hidden_states * torch.rsqrt(variance + self.variance_epsilon)
        # convert into half-precision if necessary
        # 计算 (variance + epsilon) 的平方根倒数，然后乘以原始的 hidden_states，实现归一化。
        if self.weight.dtype in [torch.float16, torch.bfloat16]:
            hidden_states = hidden_states.to(self.weight.dtype)

        return self.weight * hidden_states

# 用于生成旋转位置嵌入的正弦和余弦缓存值。这些值随后会应用到查询（query）和键（key）向量上，以引入位置信息。
class LlamaRotaryEmbedding(torch.nn.Module):
    def __init__(self, dim, max_position_embeddings=2048, base=10000, device=None):
        """
        •	dim: 位置嵌入的维度，通常等于注意力头的维度。
        •	max_position_embeddings: 支持的最大序列长度，默认值为 2048。
        •	base: 用于计算倒数频率的基数，默认值为 10000。
        •	device: 设备（CPU 或 GPU），默认为 None。
        """
        super().__init__()
        # 计算倒数频率 inv_freq 作用：inv_freq 是一组倒数频率，用于生成位置编码的角频率。
        inv_freq = 1.0 / (base ** (torch.arange(0, dim, 2).float().to(device) / dim))
        self.register_buffer("inv_freq", inv_freq)
        # 预计算频率矩阵
        self.max_seq_len_cached = max_position_embeddings
        t = torch.arange(self.max_seq_len_cached, device=self.inv_freq.device, dtype=self.inv_freq.dtype)
        # 使用爱因斯坦求和约定，计算位置索引与倒数频率的外积，得到大小为 (max_seq_len_cached, dim/2) 的频率矩阵 freqs
    ...
    def forward(self, x, seq_len=None):
        ...
        if seq_len > self.max_seq_len_cached:
            ...
            # 将计算结果注册为模块的缓冲区，以避免在每次前向传播时重复计算
            self.register_buffer("cos_cached", emb.cos()[None, None, :, :], persistent=False)
            self.register_buffer("sin_cached", emb.sin()[None, None, :, :], persistent=False)
        return (
...
def rotate_half(x):
    ...
    函数对输入张量的后一半维度进行旋转（取反并交换位置），这是旋转位置嵌入的关键步骤。
    """
    x1 = x[..., : x.shape[-1] // 2]
    x2 = x[..., x.shape[-1] // 2:]
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids):
    """旋转位置嵌入应用到查询（q）和键（k）向量上，赋予它们位置信息。
    """
...
# LlamaMLP 类实现了 LLaMA 模型中的 MLP 层。MLP 层在 Transformer 的每个块中，用于对隐藏状态进行非线性变换，增强模型的表示能力
class LlamaMLP(nn.Module):
    def __init__(
        ...
        """
        •	hidden_size: 输入和输出的隐藏状态维度。
        •	intermediate_size: 中间层的维度，通常比 hidden_size 大，增加模型的容量。
        •	hidden_act: 激活函数的名称，在 ACT2FN 字典中查找对应的激活函数。
        """
        super().__init__()
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        ### 如果开启了 LORA，将 LORA 层的输出加到 q，k，v 上
        if self.add_lora:
        ...
        ### 只需要知道 注意力分数=softmax(q*k^T/sqrt(d_k))，
